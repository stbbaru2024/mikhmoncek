...
if ($currency == in_array($currency, $cekindo['indo'])) {
	$title = array("Status Voucher", "Masukkan Kode Akses", "Paket", "Lama Terhubung", "Pemakaian Data", "Sisa Data", "Masa Aktif", "Dari", "Sampai", "tidak terdaftar.", "sudah kadaluarsa.", "Tanggal", "Cek Status", " Hari", " Jam", "Aktif", "Expired/Offline", "Kode User", "Silahkan kode akses diperpanjang");
} else {
...
<h3 class="text-center">Status Kode Anda</h3>
...
if (isset($_POST['nama'])) {
	...
	if ($API->connect($iphost, $userhost, decrypt($passwdhost))) {
		...
		$exp = $getuser[0]['comment']; // Ambil tanggal dari komentar
	...
	if ($user == "" || (substr($exp,3,1) != "/" && substr($exp,6,1) != "/")) {
		echo "<h3 class='text-center'>User Code <i style='color:#008CCA;'>$name</i> $title[9]</h3>";
	} elseif ($limitup == "1s" || $uptime == $limitup || $getbyteo == $limitbyte) {
		echo "<h3 class='text-center'>User Code <i style='color:#008CCA;'>$name</i> $title[10]</h3>";
	}
	if ($user == "" || (substr($exp,3,1) != "/" && substr($exp,6,1) != "/")) {
	} else {
		?>
...
<div class="card-header">
  ...
  <?php
	...
	if ($limitup == "1s" || $uptime == $limitup || $getbyteo == $limitbyte) {
		...
		echo "	</tr>";
		echo "</table>";
		echo "<h4><center style='color:#008CCA;'>$title[18]</center></h4>";
		echo "</div>";
	} else {
		echo "	<tr>";
		echo "		<td >$title[5]</td>";
